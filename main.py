...
def open_song(path):
	html = '<!DOCTYPE html>' \
			 ...
			 '</body>' \
			 '<script type="text/javascript" src="{1:s}/song_data.js"></script>' \
			 '<script type="text/javascript" src="JS/objects_def.js"></script>' \
			 '<script type="text/javascript" src="JS/utils.js"></script>' \
			 '<script type="text/javascript" src="JS/Item.js"></script>' \
			 '<script type="text/javascript" src="JS/Coin_ground.js"></script>' \
			 '<script type="text/javascript" src="JS/Coin_lv1.js"></script>' \
			 '<script type="text/javascript" src="JS/Coin_lv2.js"></script>' \
			 '<script type="text/javascript" src="JS/Coin_lv3.js"></script>' \
			 '<script type="text/javascript" src="JS/Item_2x.js"></script>' \
			 '<script type="text/javascript" src="JS/Item_5x.js"></script>' \
			 '<script type="text/javascript" src="JS/Item_10x.js"></script>' \
			 '<script type="text/javascript" src="JS/param.js"></script>' \
			 '<script type="text/javascript" src="JS/Score_manger.js"></script>' \
...
def create_song(mp3_path, song_data_path):
	# Initialization
	...
	mp3.export(song_data_path + "song.mp3", format="mp3")
	del mp3
	audio, fs = sf.read(path)
	if audio.ndim == 2:
		audio = audio[:fs * 10, 0]  # Keep only left channel

	# Parameters
	win_length_time = 0.07

	n_bands = 6
	bands_borders = np.array([20, 130, 400, 1500, 5000, 10000, 20000])
	first_mid_band = 1
	first_high_band = 4
	...
	for b in range(n_bands):
		z_init_matrix[:, b] = np.power(z_init_matrix[:, b] / (z_init_matrix[:, b].max() * 0.9), sanity_ground)
	...
	# Write files
	print("\nWriting...")
	file = open(song_data_path + "song_data.js", "w")
...
class App:
	...
	def reload(self):
		...
		self.frame = tk.Frame(self.master)
		self.frame.pack()
		folders = [f for f in os.listdir("SONGDATA/")]
		labels = []
		play_buttons = []
		frames = []
		for i in range(len(folders)):
			...
			play_buttons[i].pack(fill=tk.Y, side=tk.RIGHT)
			labels.append(tk.Label(frames[i], text=folders[i]))
			labels[i].pack(fill=tk.Y, side=tk.RIGHT)
...
class New_song:
	...
	def create_song(self):
		...
		if not (self.filename == "No file selected!" or self.artist == "" or self.title == ""):
			create_song(self.filename, "SONGDATA/" + self.artist + " - " + self.title + "/")
			self.root.reload()
			self.close_window()
		else:
	...
	def reload(self):
		...
		self.createButton = tk.Button(self.buttons_frame, text='Create Song', width=25, command=self.create_song)
