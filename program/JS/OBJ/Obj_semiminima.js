class Obj_semiminima extends Obj {
    constructor(gl){
        ...
        let fs_text = `#version 300 es
            ...
            void main() {
                ...
                float h = 0.15;
                float s = 1.0 - clamp( pow(nNormal.z, 25.0) + pow(bassIntensity, 3.0), 0.0, 1.0);
                float v = 0.5 + clamp( pow(nNormal.z, 25.0) + pow(bassIntensity, 3.0), 0.0, 0.4);
                vec3 color = clamp(hsv2rgb_smooth(vec3(h,s,v)), 0.0, 1.0);
                outColor = vec4(color, 1.0);
            }`;
        let program = utils.create_program(gl, vs_text, fs_text);
        let objects = [utils.parseObjText(obj_semiminima_text, false)];
